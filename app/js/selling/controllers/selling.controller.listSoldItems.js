 ...
 */
htsApp.controller('selling.controller.listSoldItems', ['$scope', 'sellingFactory', 'splashFactory', '$state', '$modal', 'newPostFactory', 'Session', function ($scope, sellingFactory, splashFactory, $state, $modal, newPostFactory, Session) {

    var userObj = Session.userObj;
    ...
    $scope.openSplash = function(elems){
        ...
        $state.go('selling.splash', { id: elems.result.postingId });
    };
    ...
    sellingFactory.getUsersItemsForSale(userObj.user_settings.name).then(function (response) {
